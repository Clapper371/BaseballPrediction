...
#lstm에 넣을 데이터니까 모델에 넣을 데이터니까 문자들이 되게 많다.-> 팀, 선수들-> id화 숫자들로 ex) 선수 포지션: 중견수=13_1 
#일렬화 직렬화 여기서는 팀으로 사용된다 0번째부터 9번까지 야구팀은 10개니까
...
def serialization(raw_data, target_column):
    ...
    return raw_data
#9월 30일에 이틀을 더 더하고 싶다 9월의 마지막이 30인지 31인지 계산하기 위해 slicing data와 묶어서 data를 자른거는 좋은데 
#언제부터 시작하고 언제부터 끝나는지 알아서 계산 이런거를 도와주는 python package datetime
...
def slicing_data(start_date, end_date, raw_data):
    ...
    if int(to_drop.head(1)) < start_date:
        ...
        #3월부터 6월까지 데이터가 있는데 start 데이터가 7월인 경우 다잘라야한다. 반환할 값이 없다.
        else:
    ...
    #kbo가 10월에 끝나는데 end date를 12월로 했을 때 else 구문 tail구문 꼬리부분을 불러와
    else:
...
#drop할 인덱스를 생성한다. 진짜 자른게 아니라 인덱스만 갖고온거 표시만 한거다.
    ...
    return raw_data
#ex) test를 7월달로 하고 싶은데 1월~6월은 학습데이터 7월은 테스트 데이터로
...
#하드코어되있는데 2019->2018로 바꾸면 된다.
if __name__ == '__main__':
    game_data_url = '2022_KBO_경기 데이터_동명이인 처리_N차전.csv'
    ...
    data =[[0, 0],
        ...
        [0, 0],
        [0, 0]]
    score_frame = pd.DataFrame(data, index=['KIA', '두산', 'NC', 'KT', 'SSG', '삼성', '롯데', '한화', 'LG', '키움'] , columns=['Runs_scored', 'Runs_allowed'])
    print(data)
    print(score_frame)
    ...
    # 날짜정보가 없는 데이터는 제거(날짜정보가 없는 경우가 간혹 있다.)
...
#일반적인 경기는 하루에 5경기인데  
#이 모델은 하나의 경기가 경기 시작 전경기와 ,경기 시작 후 경기로 분석된 모델이다.
#경기전에는 선발선수 정보밖에 없다.
    # 추가 투입 선수 여부 열 생성
    game_data.loc[:, 'only_starter'] = 0 #FALSE
    #컬럼 onlystarter라고 starter만 있는 경기인지 starter와 교체선수가 있는 경기인지 구분하는 거 생성
    # 선발선수만으로 이루어진 데이터 생성
    game_data_with_starter = game_data.replace(0.5, 0) #교체선수를 빼버린거다. 교체선수가 없는 경기들만 쭉 나열되어있다.
    #교체선수들이 없는 경기는 없다. 그냥 모델에 부여한거다.
    # 추가 투입 선수 여부 열 갱신
    # 데이터 프레임이 2가지 :game_data 데이터프레임, game_data with_데이터프레임
    game_data_with_starter.loc[:, 'only_starter'] = 1#선발 라인업만 있다. #true=1
    # 선발 선수만으로 이루어진 데이터의 경우 모든 점수 데이터 0 으로 변경  
    #game_data_with 데이터프레임은 pre게임으로 쓰인다. 그러면 스코어에 대해서 몰라야 하기때문에 
    #score 0:0부여 
    game_data_with_starter.loc[:, 'home_score'] = 0
    game_data_with_starter.loc[:, 'away_score'] = 0
    # 병합
    #데이터프레임을 위아래로 병합을 할거다.
    game_data = pd.concat((game_data, game_data_with_starter), axis=0) #axis=0->row방향으로
    #모든 경기정보가 있는 거를 위로 starter있는 경우를 밑으로
    game_data.reset_index(drop=True, inplace=True)

    # 날짜별 오름차순 정렬. 이후, 시간 순서 정리. only_starter == 1 인 데이터가 우선 발생(only_starter 내림차순 정렬)
    #lstm에 pre게임 한번 post게임 한번 이렇게 넣어준다.
    game_data = game_data.sort_values(['날짜', 'only_starter'], ascending=[True, False])
    #조건을 두개를 더 줘서 정렬을 한번 더한다. 날짜순으로 정렬 이것만 주면 pre게임인지 post게임인지 명확하게 정렬을 보장x
    #onlystarter starter 만 있으면 1 교체까지 있으면 0
    #우선순위를 날짜순으로 정렬하지만 하루 5경기가 10경기로 묶인다.
    #날짜순으로 오름차순하고  onlystarter 내림차순 1먼저 0
...
#여기 까지 전처리 마치고 학습을 했을 때는 예측률이 60이 나왔다.
#추가 부분: 전체 승률과 팀당 승률 추가 
#경기 시작 전 승률이 있고 경기 끝난 후 승률 계산해서 넣어 주자 
    # 목표 팀을 기준으로 경기 데이터 생성
    #홈팀이든 어웨이팀이든 참여한 경기를 갖고 와라 
    for target_team in team_list:
        ...
        # print(merged_game_data[['날짜','home team','away team']])
        # pd.set_option('display.max_rows', 10)
        # pd.set_option('display.max_columns', 10)

        # 팀별 득실 계산
        # for index in merged_game_data.index:
        #     print(merged_game_data.loc[index, 'home team'])
        #     if merged_game_data.loc[index, 'home team'] == target_team:
        #         team_name = merged_game_data.loc[index, 'home team']
        #         score_frame.loc[team_name, 'Runs_scored'] += merged_game_data.loc[index, 'home_score']
        #         score_frame.loc[team_name, 'Runs_allowed'] += merged_game_data.loc[index, 'away_score']
        #     elif merged_game_data.loc[index, 'away team'] == target_team:
        #         team_name = merged_game_data.loc[index, 'away team']
        #         score_frame.loc[team_name, 'Runs_scored'] += merged_game_data.loc[index, 'away_score']
        #         score_frame.loc[team_name, 'Runs_allowed'] += merged_game_data.loc[index, 'home_score']
        ...
        k_merged_game_data.loc[
            (k_merged_game_data['away team'] == target_team) & (k_merged_game_data['away result'] == 1), str(
                target_team) + '_win'] = 1
#기준이 바뀜 어웨이 홈팀이 기준이 아닌 이 팀이 이겼냐 졌냐로 
        ...
        for index in merged_game_data.index:  
            ...
            if index == 0:
                ...
                merged_game_data.loc[index, str(target_team) + '_pythagorean'] = 0.5
                k_merged_game_data.loc[index, str(target_team) + '_pythagorean'] = 0.5
            # 한 경기라도 진행됐다면
            else:
                # 이전 경기까지의 승률 할당
                ...
                # 피타고리안 승률
                merged_game_data.loc[index, str(target_team) + '_pythagorean'] = (
                    score_frame.loc[team_name, 'Runs_scored'] ** 1.83) / (
                    (score_frame.loc[team_name, 'Runs_scored'] ** 1.83) + (
                    score_frame.loc[team_name, 'Runs_allowed'] ** 1.83))
                k_merged_game_data.loc[index, str(target_team) + '_pythagorean'] = (
                    score_frame.loc[team_name, 'Runs_scored'] ** 1.83) / (
                    (score_frame.loc[team_name, 'Runs_scored'] ** 1.83) + (
                    score_frame.loc[team_name, 'Runs_allowed'] ** 1.83))
            print(score_frame)
        ...
        column_list.insert(merged_game_data.columns.get_loc(str(target_team) + '_win'),
                           column_list.pop(column_list.index(str(target_team) + '_pythagorean')))
        column_list.insert(merged_game_data.columns.get_loc(str(target_team) + '_win'),
        ...
        print(str(target_team) + ' - 전체 승률, 피타고리안 승률 계산 완료')
    ...
    # 팀별 전체 승률과 피타고리안 승률 갱신
    for team in team_list:
        for index, overall_odds, pythagorean in zip(range(0, len(full_game_merged_data)),
                                                      full_game_merged_data[str(team) + '_overall_odds'],
                                                      full_game_merged_data[str(team) + '_pythagorean']):
            latest_overall_odds = 0.5
            latest_pythagorean = 0.5
            if overall_odds != -1:
            ...
            if pythagorean != -1:
                latest_pythagorean = pythagorean
            else:
                full_game_merged_data.loc[index, str(team) + '_pythagorean'] = latest_pythagorean
    print('처리된 데이터 팀별 전체 승률, 피타고리안 승률 갱신 완료')
    ...
    # win, h_score, a_score, pythagorean, overall_odds 제외
    for target_team in team_list:
        dup_column_list.remove(str(target_team) + '_win')
        dup_column_list.remove(str(target_team) + '_pythagorean')
    ...
    end_date_train = datetime.datetime.strptime('20220531', '%Y%m%d')
    ...
    # 평가 데이터의 종료 날짜 임의 지정
    end_date_test = datetime.datetime.strptime('20221011', '%Y%m%d')
