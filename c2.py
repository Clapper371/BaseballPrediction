...
# 학습 과정에서 모델 파라미터 저장할 때 호출하는 함수
# save_model(sess, 스텝 숫자) 형태로 호출
...
if __name__ == '__main__':
    ...
    while pred_index < y_test.shape[0]:
        ...
        with tf.compat.v1.Session() as sess:
            ...
            for step in range(train_step):
                ...
                print('\rstep :', step, '학습 경과 시간 : %.2f 초' % (step_end_time - train_start_time),
    ...
    result_df.to_csv('lstm 경기 예측 요약_피타고리안.csv', encoding='ms949')
    stoc_column_list = list(stoc_result.columns.values)
    stoc_column_list.sort()
    stoc_result = stoc_result.loc[:, stoc_column_list]
    stoc_result.to_csv('lstm 경기 예측율 집계_피타고리안.csv', encoding='ms949')
